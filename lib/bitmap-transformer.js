...
module.exports = class BitmapTransform {
  ...
  transform(fn, callback) {
    ...
    pixReader.on('color', pixel => {
      ...
      this.buffer.writeUInt8(transformedPixel.b, pixel.offset);
      this.buffer.writeUInt8(transformedPixel.g, pixel.offset + 8);
      this.buffer.writeUInt8(transformedPixel.r, pixel.offset + 16);
      // this.buffer[pixel.offset] = transformedPixel.join('')
    });
    pixReader.once('end', () => {
